...
class LineChartBuilderService extends D3PlusChartBuilderService {
    ...
    prepareChart(viz, slicedDS, containerId, options, additionalOptions) {
        ...
        if (options.x_options && (options.x_options.labelInterval || options.x_options.labelMaxNumber)) {
            let labels = [];
            let interval = options.x_options.labelInterval ? options.x_options.labelInterval : Math.ceil(slicedDS.length/options.x_options.labelMaxNumber);
            if (interval > 1) {
                let index = 1;
                for (let row of slicedDS){
        ...
        if (options.y_options && (options.y_options.labelInterval || options.y_options.labelMaxNumber)) {
            let labels = [];
            let interval = options.y_options.labelInterval ? options.y_options.labelInterval : Math.ceil(slicedDS.length/options.y_options.labelMaxNumber);
            if (interval > 1) {
                let index = 1;
                for (let row of slicedDS){
                    if (index == 1 || index%interval == 0){
