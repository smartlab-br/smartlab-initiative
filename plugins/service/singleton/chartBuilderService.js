export class ChartBuilderService {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (container) {
          ...
          if (Builder) {
            try {
              const chart = (new Builder()).generateChart(containerId, dataset, options, additionalOptions)
              resolve(chart)
            } catch (err) {
              reject(err)
            }
  ...
  regenerateChart (chartHandler, type, containerId, dataset, options, additionalOptions = {}) {
