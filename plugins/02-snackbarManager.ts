...
export default defineNuxtPlugin((context: any) => {
  ...
  const buildChartAdditionalOptions = (store: MainStore, chartType: string, structure: any, chartOptions: any, dataset: any, metadata: any, sectionIndex: number = 0) => {
  ...
  const chartGen = (store: MainStore, id: string, chartType: string, structure: any, chartOptions: any, dataset: any, metadata: any, sectionIndex: number = 0) => {
    if (structure && chartOptions && validCharts.includes(chartType)) {
      if (chartOptions.from_api) {
        ...
        const additionalOptions = buildChartAdditionalOptions(store, chartType, structure, chartOptions, dataset, metadata, sectionIndex)
  ...
  const chartRegen = (store: MainStore, chartHandler: any, id: string, chartType: string, structure: any, chartOptions: any, dataset: any, metadata: any, sectionIndex: number = 0) => {
    if (structure && chartOptions && validCharts.includes(chartType)) {
      const additionalOptions = buildChartAdditionalOptions(store, chartType, structure, chartOptions, dataset, metadata, sectionIndex)
  ...
  const obsTETooltip = async (target: any, route: RouteLocationNormalizedLoaded, store: MainStore, _tooltip_list: string[] = [], _removed_text_list: string[] = [], options: any = null) => {
    let text = ""
  ...
  const obsTITooltip = async (target: any, route: RouteLocationNormalizedLoaded, store: MainStore, _tooltip_list: string[] = [], _removed_text_list: string[] = [], options: any = null) => {
  ...
  const obsSSTTooltip = async (target: any, route: RouteLocationNormalizedLoaded, store: MainStore, _tooltip_list: string[] = [], _removed_text_list: string[] = [], options: any = null) => {
    let text: string = ""
    if (options && options.clickable) {
  ...
  const obsTDTooltip = async (target: any, route: RouteLocationNormalizedLoaded, store: MainStore, _tooltip_list: string[] = [], _removed_text_list: string[] = [], options: any = null) =>{
    let text = ""
    if (options && options.clickable) {
      text += "<p class='text-xs-right ma-0'><a href='" + TooltipBuildingService.getUrlByPlace(target.options.rowData.cd_municipio_ibge_dv, route, store) + "' class='text-primary font-weight-black'>IR PARA</a></p>"
    }
  ...
  const obsCovidMunicipioTooltip = async (target: any, route: RouteLocationNormalizedLoaded, store: MainStore, _tooltip_list: string[] = [], _removed_text_list: string[] = [], options: any = null) => {
  ...
  const obsCustomTooltip = (target: any, route: RouteLocationNormalizedLoaded, tooltip_list: string[] = [], removed_text_list: string[] = [], options: any = null) => {
    ...
    app.fillDataStructure(
      ...
      (this as any).customFunctions, (this as any).showDefaultTooltip,
  ...
  const obsCovidRegicTooltip = (target: any, _route: RouteLocationNormalizedLoaded, _tooltip_list: string[] = [], _removed_text_list: string[] = [], _options: any = null) => {
    obsCovidRegicTextTooltip(target)
  }

  const obsCovidRegicUTITooltip = (target: any, _route: RouteLocationNormalizedLoaded, _tooltip_list: string[] = [], _removed_text_list: string[] = [], _options: any = null) => {
  ...
  return {
    provide: {
      ...
      obsCovidMunicipioTooltip,
      obsCustomTooltip,
      showDefaultTooltip,
      obsCovidRegicTooltip,
      obsCovidRegicUTITooltip,
      obsCovidRegicTextTooltip
    }
  }
})