    ...
    if (currentObsId.value == "sst") {
      ...
      $fetch(UrlTransformService.getApiUrl(url))
        ...
        .catch((error) => {
          ...
          loadedOdometers.value = true // Permite renderizar mesmo com erro
        })
    } else {
      loadedOdometers.value = true // Se não for SST, não precisa carregar
    }
  } else {
  ...
  async (newValue) => {
    if (newValue) {
      if (currentObs.value && currentObs.value.obsPage) {
        parallaxFile.value = currentObs.value.obsPage.background_images[idParallaxfile.value] ?? null
        loadOdometers()
      }
    }
  }
...
// Substitui o setInterval manual e gerencia o ciclo de vida sozinha
useIntervalFn(() => {
  setParallaxFile()
}, 20000)
