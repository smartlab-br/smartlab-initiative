<template>
  <v-layout class="linked-view-card elevation-5" 
    ...
    v-on:click="blocked && $store.state.PHASE_OUT && $store.state.PHASE_OUT == 'S' ? '' : pushRoute(to, external)"
    v-on:keyup.enter="blocked && $store.state.PHASE_OUT && $store.state.PHASE_OUT == 'S' ? '' : pushRoute(to, external)">
    <v-img 
      :tabindex = "indexTab"
      :src="cmpMedia"
