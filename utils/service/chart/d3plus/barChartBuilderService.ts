...
export class BarChartBuilderService extends D3PlusChartBuilderService {
  prepareChart (viz: any, slicedDS: any, containerId: string, options: any, additionalOptions: any = {}) {
    ...
    let grafico = viz
      ...
      .label((d: any) => { // retira label se x < 0 - utilizado na pirâmide
  ...
  generateViz (options: any, additionalOptions: any) {
    ...
    if (options.color !== null && options.color !== undefined) {
      ...
      viz = viz.color((d: any) => { return (d.color !== null && d.color !== undefined) ? d.color : "#2196F3" })
    } else {
    ...
    if (options.show_x_axis !== undefined && options.show_x_axis !== null) {
      if (!options.show_x_axis) { xConfig = GeneralChartBuilderService.getTransparentXYConfig() }
    } else if (options.orientation == "vertical") {
      xConfig = GeneralChartBuilderService.getTransparentXYConfig()
    }

    if (options.show_y_axis != undefined && options.show_y_axis != null) {
      if (!options.show_y_axis) { yConfig = GeneralChartBuilderService.getTransparentXYConfig() }
    } else if (options.orientation != "vertical") {
      yConfig = GeneralChartBuilderService.getTransparentXYConfig()
    }
  ...
  // Custom functions
  sortDataset (dataset: any, sort_field: string = "nu_competencia") {
    // array temporário que armazena o índice e o valor para ordenação
    const map = dataset.map((d: any, i: any) => { return { index: i, value: parseInt(d[sort_field]) } })

    // ordenação do array temporario
    map.sort((a: any, b: any) => { return +(a.value > b.value) || +(a.value === b.value) - 1 })

    // ordenaçao do dataset baseado no array temporário ordenado
    const nuDataset = map.map((d:any) => { return dataset[d.index] })

    return nuDataset
  }

  prepareAccumData (dataset: any, options: any) {
    let accum = 0
  ...
  formatNumber (value: string | number | null | undefined, format: string, num_digits: number, multiplier: number = 1, collapse: any = null, signed: boolean = false, uiTags: boolean = true) {
    if (format == "cep") {
    ...
    value = ((typeof value === "string")? parseFloat(value): value) * multiplier
    ...
    const localeConfig: Intl.NumberFormatOptions = {
