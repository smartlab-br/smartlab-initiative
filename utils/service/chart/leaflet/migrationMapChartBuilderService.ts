...
export class MigrationMapChartBuilderService extends LeafletChartBuilderService {
  ...
  fillLayers (dataset: any, options: any) {
    ...
    // let builtCircles = {};
    const builtMarkers: number[] = []
    const defaultIcon: L.Icon = new this.L.Icon({
    ...
    const locIcon: L.Icon = new this.L.Icon({
    ...
    const limits: {min: number, max: number}[] = [{min: 0, max: 0}]
        ...
        if (each_row[options.target.lat] && each_row[options.target.long] &&
          ...
          for (const [, ident] of options.indicadores.entries()) {
            ...
            if (!(builtMarkers.includes(each_row[options.target.id]))) {
              ...
              marker.on("click", this.circleClick)
              // marker.bindPopup(each_row[options.target.name]).openPopup())
