      ...
      >
        ...
        >
          <v-layout
            :id="secao.id"
            column
            :style="'background-color:' + $colorsService.assessZebraBG(indexSecao, $vuetify.theme) + ';'"
          >
            <v-flex xs12>
              <div
...
export default {
    ...
    return {
      ...
      custom_functions: {
        ...
        get_period_from_string: function (str) {
          const reg = /de \d{4} a \d{4}/g
          return str.match(reg)
        },
        get_text_from_parentheses_ds: function (d, str) {
          const reg = /\(.*\)/
          let returnStr = String(str).match(reg)
        ...
        get_week_year: function (d, week, week_start) {
          const wee_start_ISO = new Date(week_start).toISOString().substring(0, 10)
  ...
  computed: {
    ...
    currentParallax: function () {
      return 'background-image:url("/parallax/uf/' + this.customParams.cd_uf + '.jpg"); background-position: center center; background-size: cover;'
    },
  ...
  created () {
    const tmpIdObs = this.$observatories.identifyObservatory(this.$route.path.split('/')[1])
    this.idObservatorio = tmpIdObs

    this.$yamlFetcherService.loadYaml('br/observatorio/' + tmpIdObs)
      .then((result) => {
        const scope = this.getEscopo(this.$route.params.idLocalidade)
        const auId = this.getIdLocalidadeFromRoute(this.$route.params.idLocalidade)
        const msgErro = this.getMensagemErro(this.$route.params.idLocalidade)
        ...
        if (result && result.tematicos) {
          for (const indxTematico in result.tematicos) {
            thematicDatasets.push(result.tematicos[indxTematico].dataset)
          }
        }

        const indicadoresTematicos = this.$indicatorsModel.getMultipleGlobalDatasets(thematicDatasets, scope, auId)
        ...
        if (this.$route.query.compare) {
          const indicadoresTematicosCompare = this.$indicatorsModel.getMultipleGlobalDatasets(thematicDatasets, compareScope, compareAuId, '_compare')
  ...
  methods: {
        ...
        if (!section.divider) {
          for (const cardIndex in section.cards) {
            ...
            if (document.getElementById('anchor_' + section.cards[cardIndex].id) != null) {
              const { top, bottom } = document.getElementById('anchor_' + section.cards[cardIndex].id).getBoundingClientRect()
    ...
    keepLoading () {
      ...
      if (this.$route.params.idLocalidade == 0) { // Brasil
        // this.selectCoords("/topojson/country.json");
        ...
        // this.selectCoords("/topojson/regiao.json");
    ...
    loadLayout (idLocalidade, idDimensao, idObservatorio = null) {
      ...
      const escopo = this.getEscopo(this.idLocalidade)
      let observatorioDir = ''
    ...
    setDimension (content) {
      ...
      const datasetPromises = []
      const indicadoresTematicos = this.$indicatorsModel.getMultipleGlobalDatasets(thematicDatasets, escopo, this.idLocalidade)
      if (indicadoresTematicos instanceof Promise || indicadoresTematicos.then) { datasetPromises.push(indicadoresTematicos) }

      if (this.$route.query.compare) {
        const indicadoresTematicosCompare = this.$indicatorsModel.getMultipleGlobalDatasets(thematicDatasets, escopo, this.idLocalidade_compare, '_compare')
        if (indicadoresTematicosCompare instanceof Promise || indicadoresTematicosCompare.then) { datasetPromises.push(indicadoresTematicosCompare) }
      }
    ...
    fetchDataLocalidade (idLocalidade, nm_var = 'localidade') {
      const currentPlace = this.$analysisUnitModel.getPlace(nm_var)
        ...
        if (idLocalidade == 0) { // Brasil
          ...
          this.$axios(this.$axiosCallSetupService.getAxiosOptions(url))
            .then((result) => {
              ...
              localidade.tipo = 'UF'
              localidade.img = '/thumbs/municipios/' + idLocalidade + '.jpg'
    ...
    setMasterIndicator (base_object_list, rules, structure, metadata) {
      const masterVar = (metadata && metadata.indicator_var) ? metadata.indicator_var : 'masterIndicator'
    ...
    setActiveDim (idLocalidade, idObservatorio = null, idDimensao = null) {
      for (const indxDim in this.dimensoes) {
    ...
    changeDim (idDimensao, idLocalidade, idObservatorio) {
      let urlComplemento = ''
      const viewEndpoint = this.$route.query.compare ? 'localidadecompare' : 'localidade'
    ...
    getLeadSlice (rowIndx) {
      const startIndx = (rowIndx - 1) * this.ind_principais_per_row
    ...
    navDim (delta) {
      if (delta === 0) {
        ...
        for (const dimIndx in this.dimensoes) {
    ...
    goToRouteHash () {
      const el_id = this.$route.hash.replace('#', 'anchor_')
      const _this = this
      let i = 0
      const existCondition = setInterval(function () {
        const el = _this.$el.querySelector('#' + el_id)
        if (el || i == 30) {
