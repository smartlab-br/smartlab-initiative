    ...
    <v-container id="smartmap" v-if="observatorio && observatorio.prevalencia" fluid ma-0 pa-0
      ...
      <v-layout row wrap>
        ...
        <v-layout style="position:relative">
            ...
            <v-layout v-show="mapEnabled" style="position:absolute;z-index:2;right:10px" class="cursor-pointer pa-3 justify-end subheading" v-on:click="$navigationManager.pushRoute($router,'/'+$observatories.identifyObservatoryById(idObservatorio)+'/smartmap')">
          ...
          <v-flex xs12>
            <v-layout v-if="observatorio && observatorio.ranking_cards" row wrap pb-2>
              <FLPORankingList v-for="(ranking, index) in observatorio.ranking_cards" :key="index"
  ...
  export default {
    ...
    computed: {
      currentParallax: function() {
        return "background-image:url('/parallax/" + this.parallaxFile + "');";
      }
    },
    methods: {
