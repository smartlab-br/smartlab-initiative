...
export default {
  ...
  methods: {
    fillFromDataset (sourceDS, rules, sourceStructure, addedParams = null, metadata = null) {
      ...
      const allSeries = []
      const series_first_cat = {}
      const series_last_cat = {}
      ...
      fromSource: for (const row of sourceDS) {
        if (row[sourceStructure.series_field] == undefined) {
          ...
          const entry = { id: sourceStructure.series_field, cat_value: parseInt(row[sourceStructure.category_field]), value: entryValue }
      ...
      for (const row of dataset) {
        for (const series_value of seriesList) {
          ...
          if (series) {
            ...
            row['fmt_higher_value_' + series_value] = this.$numberTransformService.formatNumber(higher_value, 'inteiro') + ' (' + higher_cat + ')'
            ...
            if (last_year_value !== undefined && last_year_value !== 0 &&
              ...
              if (rate < 1) {
                row['fmt_last_rate_' + series_value] = '-' + this.$numberTransformService.formatNumber((1 - rate) * 100, 'real') + '%'
                row['last_rate_' + series_value] = (1 - rate) * 100 * -1
              } else if (rate > 1) {
                row['fmt_last_rate_' + series_value] = '+' + this.$numberTransformService.formatNumber((rate - 1) * 100, 'real') + '%'
                ...
                row['last_rate_' + series_value] = 0
                            ...
                            last_2_year_value !== undefined && last_2_year_value !== 0) {
              row['fmt_last_rate_' + series_value] = 'de ' + this.$numberTransformService.formatNumber(last_2_year_value, 'inteiro') + ' para 0'
          ...
          for (const header of this.structure.headers) {
            if (header.format) {
              row['fmt_' + header.value] = row[header.value] != ''
                ? this.$numberTransformService.formatNumber(
    ...
    addHeadersLabels (allSeries) {
      ...
      for (const serie of allSeries) {
        const last_2_cat = this.last_cat[serie] - 1
      ...
      for (const reg of hierarchicalDS) {
        for (const serie of allSeries) {
          ...
          for (const item of reg[serie]) {
            ...
            if (day_before) {
              const days_diff = Math.floor((new Date(item.cat_value) - day_before) / _MS_PER_DAY)
