<template>
  <v-flex :class="cls ? cls : 'xs12'">
    <v-layout v-if="structure.look !='minicard'" column wrap ml-2 mb-2>
      <v-flex x12 px-0 :class="'display-1-obs ranking-list-title pb-2'" v-html="title" />
      <v-flex v-if="errorMessage" x12 px-0 :class="'display-1-obs ranking-list-text'+ (structure.text_size? '-'+structure.text_size:'')+' pb-2'">
        {{ errorMessage }}
      </v-flex>
      <v-flex v-for="(item, itemIndx) in ranking" :key="itemIndx" xs12 class="ranking-list pa-0">
    ...
    <v-layout
      ...
      ml-2
      mb-2
      class="ranking-list-minicard"
    >
      <v-flex v-if="errorMessage" x12 px-0 :class="'display-1-obs ranking-list-minicard-text'+ (structure.text_size? '-'+structure.text_size:'')+' pb-2'">
        {{ errorMessage }}
      </v-flex>
      <v-flex v-for="(item, itemIndx) in ranking" :key="itemIndx" xs12 class="ranking-list-minicard pa-0">
      ...
      <v-flex x12 px-0 :class="'ranking-list-minicard-title pb-2'" v-html="title" />
    </v-layout>
  </v-flex>
</template>
