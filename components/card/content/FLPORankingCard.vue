      ...
      <v-card-title :class="color + ' ranking-card-head white--text'">
        <div style="width:100%">
          <v-layout row align-center>
            <v-flex class="ranking-card-text">
              {{ ranking[0].localidade }}
            </v-flex>
            <v-flex class="ranking-card-value">
              {{ $numberTransformService.formatNumber(ranking[0].vl_indicador,"inteiro") }}
            </v-flex>
          </v-layout>
          <div class="ranking-card-title pt-3">
            {{ title }}
          </div>
        </div>
      </v-card-title>
          ...
          <template v-for="(item, itemIndx) in ranking.slice(1)">
            <v-list-tile :key="item.localidade">
              ...
              <v-list-tile-action class="caption">
                {{ $numberTransformService.formatNumber(item.vl_indicador,"inteiro") }}
              </v-list-tile-action>
            </v-list-tile>
            <v-divider :key="itemIndx" />
          </template>
        </v-list>
      </v-card-text>
    </v-card>
