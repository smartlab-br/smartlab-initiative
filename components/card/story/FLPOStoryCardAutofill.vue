<template>
  <v-layout v-if="resultStructure" pa-0 ma-0>
...
export default {
  ...
  created () {
    const tmpStruct = this.structure

    for (const indx in this.customParams.deck) {
      const currentAutocard = this.customParams.deck[indx]
      if (tmpStruct.cd_indicador == currentAutocard.cd_indicador_spai) {
        ...
        if (!tmpStruct.type || tmpStruct.type != 'multiple-charts') {
          ...
          const minicardBlock = { type: 'minicards', cards: [] }
