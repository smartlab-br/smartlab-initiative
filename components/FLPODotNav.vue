  ...
  <v-container class="left_nav pa-0" fluid grid-list-lg>
    <v-list class="text-xs-center py-0">
      <template v-for="section in sectionsStructure">
        ...
        <v-list-tile v-if="!section.divider" :key="section.name" ripple class="pa-0">
          <v-list-tile-content class="pa-0">
            <a @click="scrollTo('anchor_' + section.name)">
              <v-tooltip right>
                <v-icon
                  slot="activator"
...
export default {
  ...
  methods: {
    ...
    buildStruct () {
      ...
      for (const groupIndex in this.sections) {
        ...
        for (const itemIndex in this.sections[groupIndex].cards) {
    ...
    assessVisibleCards () {
      ...
      for (const ancoraIndex in this.sectionsStructure) {
        if (!this.sectionsStructure[ancoraIndex].divider) {
          if (document.getElementById(this.sectionsStructure[ancoraIndex].name) != null) {
            const { top, bottom } = document.getElementById(this.sectionsStructure[ancoraIndex].name).getBoundingClientRect()
