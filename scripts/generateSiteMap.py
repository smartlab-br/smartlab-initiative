...
def generate_structure(view_conf_path):
    ...
    obs = [] 
    for item in obs_yaml['observatories']:
        if not item['external'] and not item['blocked']:
            obs.append({'id':item['id'], 'item_type':'observatorio', 'title':item['title'], 'short_title':item['short_title'], 'url': item['to'], 'search_text': item['title'], 'app_icon': item['app_icon']})
    obs_dim = []
    # Observatórios
    for obs_item in obs:
        ...
        obs_search_text = 'Apresentação - ' + obs_item['title']
        print(obs_item['title'])
        ...
        dimensions.append({'id': obs_item['id'] + '_apresentacao', 'item_type':'obs_apresentacao','title': 'Apresentação - Observatório ' + obs_item['short_title'], 'url': obs_item['url'], 'search_text': obs_search_text.replace('<br/>', ' ')})

        # Smartmap search_text
        obs_search_text = 'Smartmap - Mapa avançado - ' + obs_item['title']
                        ...
                        for minicard in item['minicards']:
                            for arg in minicard['args']:
                                if (arg['prop'] != 'value'):
                                    ...
                                    if ('template' in arg):
                                        obs_search_text += ' ' + arg['template']
        dimensions.append({'id': obs_item['id'] + '_smartmap', 'item_type':'obs_smartmap','title': 'Smartmap - Mapa avançado - Observatório ' + obs_item['short_title'], 'url': obs_item['url'] + '/smartmap', 'search_text': obs_search_text.replace('<br/>', ' ')})

        # Dimensões
        for dim_item in dimensions_yaml:
            if (not 'external' in dim_item or ('external' in dim_item and not dim_item['external'])) and not dim_item['blocked']:
                ...
                for section in dim_yaml['secoes']:
                    ...
                    for card in section['cards']:
                        ...
                        cards.append({'id': card['id'], 'item_type':'card', 'title': card['title']['fixed'], 'url': dim_item['to'] + '#' + card['id'],'search_text': card_search_text.replace('<br/>', ' ')})
                dimensions.append({'id': dim_item['id'], 'item_type':'dimensao','title':dim_item['short_desc'], 'url': dim_item['to'], 'search_text': dim_item['label'], 'children': cards})
...
try:
    view_conf_path =  str(pathlib.Path().resolve()).replace('scripts','public') + '/smartlab-initiative-viewconf/br'   
